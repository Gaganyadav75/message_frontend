...
export const UnblockChatApiCall = createAsyncThunk<addChatResponseType,addChatType, { rejectValue: string }>('auth/unblock', async (contact, thunkAPI) => {
    try {
      const response = await fetch(`${BACKEND_URL_API}/chat/unblock`, {
        method:"POST",
        credentials: 'include',
