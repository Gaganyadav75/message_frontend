...
export const BlockChatApiCall = createAsyncThunk<addChatResponseType,addChatType, { rejectValue: string }>('auth/block', async (contact, thunkAPI) => {
    try {
      const response = await fetch(`${BACKEND_URL_API}/chat/block`, {
        method:"POST",
        credentials: 'include',
